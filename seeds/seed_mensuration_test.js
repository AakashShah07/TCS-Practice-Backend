...
const seed = async () => {
  try {
    ...
    // Get all Mensuration questions
    const questions = await Question.find({ topic: 'Mensuration' }).select('_id');
    console.log(`Found ${questions.length} Mensuration questions`);

    // Remove existing Mensuration test if any
    await Test.deleteMany({ topic: 'Mensuration', type: 'topic_practice' });
