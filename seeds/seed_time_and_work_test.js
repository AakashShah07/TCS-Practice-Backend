...
const seed = async () => {
  try {
    ...
    // Get all Time & Work questions
    const questions = await Question.find({ topic: 'Time & Work' }).select('_id');
    console.log(`Found ${questions.length} Time & Work questions`);

    // Remove existing Time & Work test if any
    await Test.deleteMany({ topic: 'Time & Work', type: 'topic_practice' });
